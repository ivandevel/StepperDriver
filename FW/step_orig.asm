...
;Initial values:
.equ	V	=300		;speed, steps per second (VMIN..4000)
.equ	A	=100		;acceleration, steps per squared second (1..4000)
...
.def	temp0	=r00		;lpm data register
		
.def	STCNTL	=r01		;step program timer low byte
.def	STCNTH	=r02		;step program timer high byte

.def	URCNTL	=r03		;update rate program timer low byte
.def	URCNTH	=r04		;update rate program timer high byte

.def	TL	=r05		;step period low
.def	TH	=r06		;step period high

.def	VRL	=r07		;required speed x 64 low
.def	VRM	=r08		;required speed x 64 medium
.def	VRH	=r09		;required speed x 64 high
...
COMP0:	push	temp                                ;Занесение регистра в стек
	in	temp,SREG                                   ;Чтение порта
	push	temp
...
COMP1:	push	temp                                ;кладем текущее значение temp на стек
...
NewPh:	clr	tempC
	...
	or	tempB,VCM
	or	tempB,VCL
	breq	Stop		;clear all phases if VC=0
